...
def generarRptoAdminCiudades(request):
    if request.method == 'POST':
        ...
        if request.body:
            ...
            # Cambiar el nombre de la plantilla o usar un archivo Excel existente
            ...
            if os.path.exists(plantilla_path):
                ...
                response['Content-Disposition'] = 'attachment; filename=Plantilla_Rpto_CuotaAdmin_Ciudades.xlsx'
...
@csrf_exempt
@require_http_methods(["POST"])
def generarRptoAdminPropietarios(request):
    if request.method == 'POST':
        # Verificar si hay datos en la solicitud
        if request.body:
            # Decodificar los datos JSON de la solicitud
...
@csrf_exempt
@require_http_methods(["POST"])
def generarRptoAdminCiudadesColi(request):
    if request.method == 'POST':
        # Verificar si hay datos en la solicitud
        if request.body:
            # Decodificar los datos JSON de la solicitud
