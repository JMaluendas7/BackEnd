...
def rptosMigracion(request):
    try:
        if request.method == "POST":
            ...
            print(Terminal, Fecha, Opcion)

            cursor = conn.cursor()

            cursor.execute("{CALL RP_MIGRACION (?, ?, ?, ?, ?)}",
...
@csrf_exempt
@require_http_methods(["POST"])
def generarRptosMigracion(request):
    if request.method == 'POST':
        ...
        if request.body:
            ...
            if Opcion == 1:
                ...
                plantilla_path = os.path.join(
                    script_dir, '../docs/Plantillas/Plantilla_Rpto_MigracionRptoXFecha.xlsx')
